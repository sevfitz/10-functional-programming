...
(function (module) {
  ...
  */
  ...
  Article.numWordsAll = () => {
    return Article.all.map(article => article.body.length).reduce((subtotal, next) => Math.round(subtotal + (next / 7.5)));
  };

  // DONE: Chain together a `map` and a `reduce` call to produce an array of unique author names. You will
  // probably need to use the optional accumulator argument in your reduce call.
  Article.allAuthors = () => {
    return Article.all.map(article => article.author).reduce((authorArray, currentAuthor) => {
      !authorArray.includes(currentAuthor) ? authorArray.push(currentAuthor) : authorArray
  ...
  Article.numWordsByAuthor = () => {
    return Article.allAuthors().map(author => {
      ...
      let authorObj = {
        name: author,
        wordCount: Article.all.filter(articleObj => articleObj.author === author).map(article => article.body.length).reduce((subtotal, next) => Math.round(subtotal + (next/ 7.5)))
      };
      return authorObj;
    })
  };
